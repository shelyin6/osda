...
| **ANTLR 4 Runtime** | **4.13.2** | **BSD-3-Clause** |

## 随仓库分发的第三方源码

| 资产 | 位置 | 许可证 | 说明 |
| --- | --- | --- | --- |
| Oracle PL/SQL 语法 | `src/main/antlr4/com/osda/parser/antlr/PlSqlLexer.g4`、`PlSqlParser.g4` | Apache-2.0 | 取自 antlr/grammars-v4（`sql/plsql`），保留原始版权头与许可证声明 |
| 语法所需基类 | `src/main/java/com/osda/parser/antlr/PlSqlLexerBase.java`、`PlSqlParserBase.java` | Apache-2.0 | 同上，仅补充 `package` 声明 |
